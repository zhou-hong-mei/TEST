...
const ajax = (url, data, successCallback, failCallback, type = "POST") => {
    ...
    // let sign = encryptedKey(data);
    //    data = {
    //        sign,
    //    }
    // data.sign = encryptedKey(data);
    // console.log(AesEncrypt(data),'哒哒哒哒哒哒')
    ...
    uni.request({
        ...
        success: (res) => {
            ...
            // console.log( res.data,'类型=+++++++++++++++++');
            // 调试时打印

            let urls = [
                '/front/mining/getList',
        ...
        fail: () => {
            let languageRadar = uni.getStorageSync('languageRadar');
            uni.showToast({
                title: languageRadar == 'zh' ? '请稍后重试' : 'Please try again later',
                icon: "none",
                position: 'bottom'
            });
        }
