<template>
	<view class="page">
       ...
       <view class="content">
           ...
           <view class="infoBox">
            <view class="boxInput">
             <view class="infoDetail">
             	<input v-model="tel" placeholder="请输入您的手机账号" placeholder-class="plClass" @focus="telfocus('tel')" @blur="getTel"/>
             </view>
             <view class="error" v-if="errorTips==1">
                 <text>手机号格式不正确!</text>
             </view>
			 <view class="error" v-if="telTips==1">
			     <text>手机号不能为空！</text>
			 </view>
             </view>
           <view class="boxInput">
             <view class="infoDetail">
             	<input v-model="code" placeholder="请输入短信验证码验证账户" placeholder-class="plClass" @focus="telfocus('code')"/>
                <view class="sendCode" @click="getCode">{{codeTip}}</view>
             </view>
             <view class="error" v-if="errorTips==4">
                 <text>请输入手机号码!</text>
             </view>
           ...
           <view class="boxInput">
             <view class="infoDetail">
             	<input v-model="pass" type="password"  @focus="telfocus('pass')" @blur="getPass" maxlength="20"  placeholder="请输入安全密码 8-20位字母和数字组合" placeholder-class="plClass" v-if="isPws"/>
                <input v-model="pass" type="text" maxlength="20" placeholder-class="plClass" v-else/>
            ...
            <view class="boxInput">
              <view class="infoDetail">
                 ...
                 <input v-model="repass" type="text" maxlength="20" placeholder-class="plClass" v-else/>
              ...
              <view class="error" v-if="errorTips==3">
                  <text>两次密码不一致！</text>
              </view>
	...
	export default {
		...
		methods: {
            ...
            getCode:function(){
				...
				//下面进行验证码的判断，如果验证吗正确返回true，如果验证码不正确给出相应提示返回false
			...
			resetPass:function(){
				...
				if(!this.getTel()||!this.getPass()||!this.getRepass()){
				    return false;
				}
			}
		}
